...
class BrainDQN:
    def __init__(self, actions):
        ...
        checkpoint = tf.train.get_checkpoint_state("saved_net_rgb1")
    ...
    def _create_tensor_board(self):
        ...
        summaries.append(tf.summary.scalar("Qcost", self.cost))

        self.summary_op = tf.summary.merge(summaries)
        self.log_writer = tf.summary.FileWriter("logs/%s" % 'DQNNETRGB1', self.session.graph)

    def createQNetwork(self):
        # network weights
        self.global_step = tf.Variable(0, trainable=False, name='step')

        W_conv1 = self.weight_variable([8, 8, 12, 32])
        ...
        stateInput = tf.placeholder("float", [None, 80, 80, 12])
    ...
    def createTrainingMethod(self):
        ...
        self.trainStep = tf.train.AdamOptimizer(1e-6).minimize(self.cost, global_step=self.global_step)

    def log(self, y_batch, action_batch, state_batch):
        step, summary = self.session.run([self.global_step, self.summary_op], feed_dict={
            self.yInput: y_batch,
    ...
    def trainQNetwork(self):
        ...
        if self.timeStep % configure.SAVE_NET == 0:
            self.saver.save(self.session, 'saved_net_rgb1/' + 'network' + '-dqn',
                            global_step=self.timeStep + self.episode)

        if configure.TENSORBOARD and self.timeStep % configure.TENSORBOARD_UPDATE_FREQUENCY == 0:
            self.log(y_batch, action_batch, state_batch)
    ...
    def setPerception(self, nextObservation, action, reward, terminal):
        ...
        if self.timeStep % 100 == 0:
            print "TIMESTEP", self.timeStep + self.episode, "/ STATE", state, \
    ...
    def setInitState_rgb(self, observation):
        self.currentState = np.concatenate((observation, observation, observation), axis=2)
