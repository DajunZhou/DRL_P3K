...
class DDQN:
    def __init__(self, model_name, action_dim, rebuffer, Trainer=False, Graph=None, Sess=None, ID=None):
    ...
    def Perce_Train(self):
        rebuffer_count = self.replaybuffer.count()
    ...
    def getAction(self):
        ...
        if self.epsilon > (FINAL_EPSILON*(self.id+1)) and self.timeStep > OBSERVE:
            self.epsilon -= (INITIAL_EPSILON - FINAL_EPSILON*(self.id+1)) / EXPLORE
