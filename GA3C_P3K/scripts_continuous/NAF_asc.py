...
class NAF():
    def __init__(self, model_name, action_dim, rebuffer, exp_queue=None, Trainer=False, Graph=None, Sess=None, ID=None):
        ...
        self.img_channels = configure.STACKED_FRAMES
        ...
        with self.graph.as_default() as g:
            with tf.device(self.device):
                ...
                with tf.variable_scope(self.model_name):
                    ...
                    self.Advantage, self.Value, self.Qout, self.predict, self.actions, self.Keep_Prob \
                        = self.__create_graph()
        ...
        with self.graph.as_default() as g:
            with tf.device(self.device):
                ...
                with tf.variable_scope('Target_net'):
                    self.imageInT, _,_,_,_,_,_, self.ValueT,_,_,_,self.Keep_ProbT = self.__create_graph()
    ...
    def __create_graph(self):
        ...
        V = self.fc_layer(fc1_drop, 1, 'Value', func=None)
        with tf.variable_scope('advantage'):
            l = self.fc_layer(fc1_drop, (self.action_dim*(self.action_dim + 1))/2, 'l', func=None)
            mu = self.fc_layer(fc1_drop, self.action_dim, 'mu', func=tf.nn.tanh)
        ...
        return imageIn, conv1, conv2, conv3, pool1, conv4, A, V, Q, mu, actions, keep_prob
    ...
    def log(self, y_batch, action_batch, state_batch):
        feed_dict = {self.targetQ: y_batch,
                     ...
                     self.Keep_Prob: configure.KEEP_PROB,
            ...
            if experiences[step_t].terminal != True:
                End_V = self.sess.run(self.ValueT,
                                           feed_dict={self.imageInT:[experiences[step_t].nextstate],
                                                      self.Keep_ProbT: configure.KEEP_PROB})
    ...
    def getAction(self):
        if np.random.rand(1) < self.epsilon:
            ...
            action_get = self.sess.run(self.predict, feed_dict={self.imageIn:[self.currentState], self.Keep_Prob:configure.KEEP_PROB})
            action_get = np.resize(action_get, (self.action_dim))
            # print action_get

        if self.epsilon > (FINAL_EPSILON+(self.id)*0.15) and self.timeStep > OBSERVE:
            self.epsilon -= (INITIAL_EPSILON - (FINAL_EPSILON+(self.id)*0.15)) / EXPLORE
