...
class ThreadAgent(Thread):
    def __init__(self, id, rebuffer, network, exp_queue):
        ...
        self.id = id
        self.Agent = NAF('Agent' + str(id), configure.actions_dim, rebuffer, exp_queue=exp_queue, Graph=network.graph,
                         Sess=network.sess, ID=id)
    ...
    def run(self):
        ...
        for episode in xrange(configure.EPISODES):
            ...
            lstm_state = self.Agent.init_lstim_state
            # copy NET
            ...
            while 1 != 0 and episode_steptime < 900:
                action, lstm_state = self.Agent.getAction(lstm_state)
                ...
                if terminal or episode_steptime >= 900:
                    print 'Agent' + str(
                        self.id), "  Total: ", total_reward, "  Rebuffer: ", self.Agent.replaybuffer.count(),
